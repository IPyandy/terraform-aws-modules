locals {
  ...
  node_user_data = "${data.template_file.eks_node_init.rendered}"
...
contexts:
- context:
    cluster: "${var.cluster_name}-${var.env}-${var.rand1}"
    user: aws
  name: "${var.cluster_name}-${var.env}-${var.rand1}"
current-context: "${var.cluster_name}-${var.env}-${var.rand1}"
kind: Config
preferences: {}
users:
- name: aws
  user:
    exec:
      ...
      args:
        - "token"
        - "-i"
        - "${var.cluster_name}-${var.env}-${var.rand1}"
KUBECONFIG
